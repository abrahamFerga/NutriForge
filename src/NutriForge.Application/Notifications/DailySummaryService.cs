...
public sealed class DailySummaryService(DiaryService diary, INotificationChannel channel)
{
    ...
    internal static string BuildMessage(DiaryDayDto day) =>
        Format(day.Date, day.Consumed.Kcal, day.Consumed.ProteinG, day.Target?.Kcal);

    /// <summary>
    /// The deterministic summary line from primitives. Shared by the on-demand endpoint and the
    /// scheduled worker, so both produce byte-identical messages.
    /// </summary>
    public static string Format(DateOnly date, double consumedKcal, double consumedProtein, double? targetKcal)
    {
        var c = CultureInfo.InvariantCulture;
        if (targetKcal is { } t)
        {
            var remaining = t - consumedKcal;
