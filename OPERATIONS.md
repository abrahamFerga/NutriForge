...
3. If no LLM key is configured, generation returns 503 immediately. This is expected; check
   `Ai__ApiKey` in App Configuration (sourced from the `openai-api-key` Aspire parameter /
...
### 3.7 AI-generated recipes accumulating in the recipe catalog

**Symptoms**: the recipe browser fills up with AI-generated recipes from catalog-level
generation (`POST /api/v1/recipes/generate`).

Note: recipes generated by auto-diet (`POST /api/v1/diet-plans/auto`) are tagged
`plan-generated` and are **never shown** in the browser — they are self-contained within
the plan and require no cleanup.

For catalog AI recipes (`SourceType = "ai-generated"`), the user can clear them via the
"Clear AI recipes" button in the SPA, or an operator can call the endpoint directly:

```http
DELETE /api/v1/recipes/ai-generated
Authorization: Bearer <user-token>
```

Returns `{ "removed": N }`. Only removes the requesting user's own AI catalog recipes.
